...
parser.add_argument('--model', type=str, default='GAT', choices=['APPNP', 'GAT', 'GCN', 'GraphSAGE', 'DAGNN'])
parser.add_argument('--heads', type=int, default=1)
parser.add_argument('--METHOD', type=str, default= 'SRGNN', choices=['SRGNN','cmd', 'MMD','CKLJS','CKL','CJS','kld','jsd','emd', None])
parser.add_argument('--TRAININGSET', type=str, default= 'Baised', choices=['Baised', None])
parser.add_argument('--BIASDATA', type=str, default=None, choices=['generate_own_bias_data',None])
...
#parser.add_argument('--ld', type=float, default=1, choices=[1,0.1,0.3,0.5,0.7])
parser.add_argument('--ld1', type=float, default=0.7, choices=[1,0.1,0.3,0.5,0.7])
parser.add_argument('--ld2', type=float, default=0.1, choices=[0,1,0.1,0.3,0.5,0.7])
...
if args.TRAININGSET == 'Baised':
    name = 'baised_train_' + args.dataset + '_' + args.model + '_' + args.conformal_score + '_ld1_'+ str(args.ld1) + '_ld2_'+ str(args.ld2)
    ...
    if args.METHOD == 'SRGNN':
        ...
        name += '_method_' + args.METHOD
    elif args.METHOD == 'CKLJS':
        name += '_method_' + args.METHOD    
    elif args.METHOD == 'CKL':
        name += '_method_' + args.METHOD
    elif args.METHOD == 'CEMD':
        name += '_method_' + args.METHOD    
    elif args.METHOD == 'CJS':
        name += '_method_' + args.METHOD
    elif args.METHOD == 'KLJS':
        name += '_method_' + args.METHOD
    elif args.METHOD == 'CKLJSEM':
        name += '_method_' + args.METHOD         
    ...
    if args.alpha_bias != 0.1:
        name += '_alpha_bias_' + str(args.alpha_bias)
else:
...
if __name__ == '__main__':
    ...
    for run in tqdm(range(args.epochs)):
        ...
        if args.BIASDATA == 'generate_own_bias_data':
            ...
            # ppr_vector = train_dump['ppr_vector']
                ...
                idx_train = utils.biased_Data(one_hot_labels, ori_idx_train, idx_val, ori_idx_test, args.dataset, ppr_dist,
                                          labels)
            elif args.alpha_bias == 0.4:
                train_dump = pickle.load(open('intermediate/{}_{}_dump.p'.format(args.dataset, args.alpha_bias), 'rb'))
            # ppr_vector = train_dump['ppr_vector']
                ppr_dist = train_dump['ppr_dist']
                idx_train = utils.biased_Data(one_hot_labels, ori_idx_train, idx_val, ori_idx_test, args.dataset, ppr_dist,
                                          labels)
            elif args.alpha_bias == 0.6:
                train_dump = pickle.load(open('intermediate/{}_{}_dump.p'.format(args.dataset, args.alpha_bias), 'rb'))
            # ppr_vector = train_dump['ppr_vector']
                ppr_dist = train_dump['ppr_dist']
                idx_train = utils.biased_Data(one_hot_labels, ori_idx_train, idx_val, ori_idx_test, args.dataset, ppr_dist,
        ...
        if (os.path.exists(model_checkpoint_base)) and (not args.base_retrain):
            ...
            for epoch in range(args.epochs_base):
                ...
                if args.METHOD == 'SRGNN':
                    ...
                    if args.size_loss:
                        ########################################
                        loss = loss.mean() + args.ld1 * model.shift_robust_output(idx_train, iid_train) \
                               + args.ld2 * model.MMD_output(idx_train, iid_train) + size_loss
                    else:
                        loss = loss.mean() + args.ld1 * model.shift_robust_output(idx_train, iid_train) \
                               + args.ld2 * model.MMD_output(idx_train, iid_train)
                    ...
                    if args.size_loss:
                        ...
                        loss = loss.mean() + args.ld1 * model.shift_robust_output(idx_train, iid_train) \
                               + args.ld2 * model.KLD_output(idx_train, iid_train) + size_loss
                    else:
                        loss = loss.mean() + args.ld1 * model.shift_robust_output(idx_train, iid_train) \
                               + args.ld2 * model.KLD_output(iid_train, idx_train)
                elif args.METHOD == 'CEMD':
                    
                    if args.size_loss:
                        
                        loss = loss.mean() + args.ld1 * model.shift_robust_output(idx_train, iid_train) \
                               + args.ld2 * model.EMD_output(idx_train, iid_train) + size_loss
                    else:
                        loss = loss.mean() + args.ld1 * model.shift_robust_output(idx_train, iid_train) \
                               + args.ld2 * model.EMD_output(idx_train, iid_train)        
                elif args.METHOD == 'CJS':
                    
                    if args.size_loss:
                        loss = loss.mean() + args.ld1 * model.shift_robust_output(idx_train, iid_train) \
                               + args.ld2 * model.JSD_output(idx_train, iid_train) + size_loss
                    else:
                        loss = loss.mean() + args.ld1 * model.shift_robust_output(idx_train, iid_train) \
                               + args.ld2 * model.JSD_output(idx_train, iid_train)            
                elif args.METHOD == 'MMD':
                    if args.size_loss:
                        loss = loss.mean() + args.ld1 * model.MMD_output(idx_train, iid_train) + size_loss
                    else:
                        loss = loss.mean() + args.ld1 * model.MMD_output(idx_train, iid_train)
                elif args.METHOD == 'cmd':
                    if args.size_loss:
                        loss = loss.mean() + args.ld1 * model.CMD_output(idx_train, iid_train) + size_loss
                    else:
                        loss = loss.mean() +  args.ld1 * model.CMD_output(idx_train, iid_train)
                elif args.METHOD == 'CKLJS':
                    if args.size_loss:
                        loss = loss.mean() + args.ld1 * model.shift_robust_output(idx_train, iid_train) \
                               + args.ld2 * model.KLD_output(idx_train, iid_train) + args.ld3 * model.JSD_output(idx_train, iid_train) \
                                  + size_loss
                    else:
                        loss = loss.mean() + args.ld1 * model.shift_robust_output(idx_train, iid_train) \
                               + args.ld2 * model.KLD_output(idx_train, iid_train) +args.ld3 *  model.JSD_output(idx_train, iid_train)
                    ...
                    if args.size_loss:
                        loss = loss.mean() + args.ld1 * model.shift_robust_output(idx_train, iid_train) \
                               + args.ld2 * model.KLD_output(idx_train, iid_train) + model.JSD_output(idx_train, iid_train) \
                                + args.ld3 * model.EMD_output(idx_train, iid_train)  + size_loss
                    else:
                        loss = loss.mean() + args.ld1 * model.shift_robust_output(idx_train, iid_train) \
                               + args.ld2 * model.KLD_output(idx_train, iid_train) + args.ld3 * model.JSD_output(idx_train, iid_train) \
                                  + model.EMD_output(idx_train, iid_train)                       
                elif args.METHOD == 'kld':
                    if args.size_loss:
                        loss = loss.mean() + args.ld1 * model.KLD_output(idx_train, iid_train) + size_loss
                    else:
                        loss = loss.mean() + args.ld1 * model.KLD_output(idx_train, iid_train)

                elif args.METHOD == 'jsd':
                    if args.size_loss:
                        loss = loss.mean() + args.ld1 * model.JSD_output(idx_train, iid_train) + size_loss
                    else:
                        loss = loss.mean() + args.ld1 * model.JSD_output(idx_train, iid_train)
                elif args.METHOD == 'emd':
                    if args.size_loss:
                        loss = loss.mean() + args.ld1 * model.EMD_output(idx_train, iid_train) + size_loss
                    else:
                        loss = loss.mean() + args.ld1 * model.EMD_output(idx_train, iid_train)        
    ...
    if not args.not_save_res:
        print('Saving results to', './pred_p/' + name + '.txt')
        with open('./pred_p/' + name + '.txt', 'w') as f:
